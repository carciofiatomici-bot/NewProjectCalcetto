...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- NUOVA LOGICA: CALCOLO DEL PUNTEGGIO TATTICO ---
    /**
     * Calcola il punteggio tattico di una squadra basato sul livello medio dei titolari.
     * @param {string} teamId - ID della squadra.
     * @returns {Promise<number>} Il punteggio tattico arrotondato.
     */
    const calculateTeamTacticScore = async (teamId) => {
        ...
        // Usiamo la funzione getFormationPlayers da interfaccia.js (la esponiamo globalmente lì)
        const titolari = window.getFormationPlayers ? window.getFormationPlayers(teamData) : teamData.formation?.titolari || [];

        // Calcoliamo il livello medio SOLO se ci sono 5 titolari
        if (titolari.length !== 5) {
            console.warn(`Squadra ${teamId} non ha 5 titolari schierati. Punteggio tattico: 1 (MINIMO).`);
            return 1; // Punteggio minimo se la formazione è incompleta
        }
        
        // Usiamo la funzione calculateAverageLevel da interfaccia.js
    ...
    const simulateNextRound = async (schedule, allTeams) => {
        ...
        try {
            ...
            for (const match of nextRound.matches) {
                ...
                // NUOVA LOGICA DI SIMULAZIONE BASATA SUL PUNTEGGIO TATTICO
                const homeScore = await calculateTeamTacticScore(match.homeId);
                const awayScore = await calculateTeamTacticScore(match.awayId);
                
                // Calcolo Gol: Maggiore è il punteggio, maggiori sono le probabilità di fare gol.
                // Usiamo un modello basato sulla differenza di punteggio per influenzare la casualità.
                const scoreDifference = homeScore - awayScore;
                
                // Base di gol (1-3) + bonus influenzato dalla differenza di punteggio.
                // Randomness: 0 to 2
                let homeGoals = getRandomInt(1, 3) + Math.round(Math.random() * 2); 
                let awayGoals = getRandomInt(1, 3) + Math.round(Math.random() * 2);
